...
public class StudentConfiguration {
	// Configuration 이라는 어너테이션이 붙은 클래스를 이용해서 활용가능한 클래스의 객체를
	// 직접 생성 리턴해 주는 메서드를 제작 운영합니다.
	...
	public DataBaseConnectionInfo dbConnectionInfo() {
		...
		return temp;
	} //Bean 어너테이션이 붙은 메서드는 해당 객체가 리턴되는 객체들이며, 함수를 직접 이름으로 호출하지 않고
	// getBean() 메서드를 이용하여 객체를 리턴 받게 환경을 구성해 줍니다.
